-- Скрипт для полного пересоздания таблиц инвентаря
-- ВНИМАНИЕ: Этот скрипт УДАЛИТ все существующие данные!
-- Используйте только если нужно начать с нуля

-- =====================================================
-- УДАЛЕНИЕ СУЩЕСТВУЮЩИХ ТАБЛИЦ (ОСТОРОЖНО!)
-- =====================================================

-- Отключаем RLS перед удалением
ALTER TABLE IF EXISTS public.tools DISABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS public.consumables DISABLE ROW LEVEL SECURITY;

-- Удаляем триггеры
DROP TRIGGER IF EXISTS trigger_tools_updated_at ON public.tools;
DROP TRIGGER IF EXISTS trigger_consumables_updated_at ON public.consumables;
...
COMMENT ON COLUMN public.tools.quantity IS 'Количество расходника (должно быть >= 0)';
