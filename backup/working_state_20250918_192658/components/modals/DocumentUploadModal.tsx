...
export const DocumentUploadModal: React.FC<DocumentUploadModalProps> = ({ onClose, onSave, showAlert, projectId = null }) => {
    ...
    const { uploadFileWithFallback, createDocument, isUploading } = useFileStorage();
    ...
    const handleSave = async () => {
        ...
        if (selectedFile.size > maxFileSize) {
            ...
            showAlert(`Файл "${selectedFile.name}" слишком большой: ${fileSizeMB}MB. Максимальный размер: 10MB`);
            return;
        }

        try {
            // Загружаем файл с fallback на base64
            const uploadResult = await uploadFileWithFallback('documents', selectedFile);
            ...
            const errorMessage = error instanceof Error ? error.message : 'Произошла ошибка при сохранении документа.';
